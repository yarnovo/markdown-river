...
项目已升级到 V2 架构，实现了乐观更新和快照驱动的渲染系统：

- **乐观解析器** ✅ - 移除 POTENTIAL 概念，直接发送确定性令牌
- **快照渲染器** ✅ - 每个令牌生成完整 DOM 快照
- **DOM 差分算法** ✅ - 计算最小化 DOM 操作
- **事件驱动架构** ✅ - 模块间通过事件总线通信
- **防止格式符号闪烁** ✅ - 乐观更新彻底解决闪烁问题
- **流式输入支持** ✅ - 完整的流式处理架构
- **代码质量保证** ✅ - 209个测试全部通过
- **指纹测试框架** ✅ - 输入输出对比测试框架
...
- 架构模块：3个核心组件（乐观解析器、快照渲染器、DOM 差分）+ 支撑模块
- 文档完整性：完整的技术文档和使用指南
- 测试框架：包含指纹测试框架
...
**背景**：
经过指纹测试发现原有的 POTENTIAL 概念过于保守，需要实现真正的乐观更新。
...
2. **快照驱动渲染**
   - 每个令牌产生完整的 DOM 快照
   - 通过 DOM Diff 计算最小更新
   - StreamingRendererV2 作为集成模块

3. **组件分层**
   - 核心层：乐观解析器、快照渲染器、DOM Diff
   - 基础设施层：事件总线、StyleProcessorV2、DOMManagerV2
   - 组件通过事件总线解耦通信

4. **数据流**
   ```
   字符输入 → 乐观解析器 → [parser:token] → 快照渲染器
           → [snapshot:updated] → DOM Diff → DOM操作
   ```

**关键文件**：

- `src/core/optimistic-parser.ts` - 乐观解析器
- `src/core/snapshot-renderer.ts` - 快照渲染器
- `src/core/dom-diff.ts` - DOM 差分计算
- `src/streaming-renderer-v2.ts` - V2 集成模块
- `src/infrastructure/style-processor-v2.ts` - V2 样式处理器
- `src/infrastructure/dom-manager-v2.ts` - V2 DOM 管理器
...
- 简单集成测试通过
- TypeScript 类型检查存在一些问题待修复
- 架构文档已更新反映最新设计

### V2 架构实现（2025-07-10）

**背景**：
用户要求修改架构，移除 POTENTIAL 概念，实现快照驱动的渲染系统。

**架构设计**：

1. **乐观解析器（Optimistic Parser）**
   - 移除所有 POTENTIAL\_\* 令牌
   - 直接发送确定性令牌（如 STRONG_START、STRONG_END）
   - 通过事件总线发送 parser:token 事件

2. **快照渲染器（Snapshot Renderer）**
   - 监听令牌事件，生成完整 DOM 快照
   - 每个令牌都会产生新的快照版本
   - 依赖注入 StyleProcessorV2 处理样式
   - 发送 snapshot:updated 事件

3. **DOM 差分（DOM Diff）**
   - 监听快照事件，计算与上一个快照的差异
   - 生成最小化 DOM 操作序列
   - 发送 dom:operations 事件

4. **集成层（StreamingRendererV2）**
   - 监听 DOM 操作事件
   - 调用 DOMManagerV2 执行实际 DOM 更新
   - 管理整个渲染生命周期

**关键决策**：

- **事件驱动架构**：所有模块通过事件总线通信，完全解耦
- **快照驱动**：每个状态都有完整快照，便于调试和时间旅行
- **乐观更新**：立即渲染预测结果，错误时修正
- **最小化 DOM 操作**：通过差分算法减少 DOM 更新

**实现挑战**：

1. **TypeScript 事件类型**：EventBus 的类型系统与自定义事件不兼容
   - 解决方案：使用类型断言 `(eventBus as any).on(...)`

2. **测试迁移**：V1 测试需要适配 V2 的乐观更新行为
   - V1：`**bo` → `<span>**bo</span>`
   - V2：`**bo` → `<strong>bo</strong>`（立即应用格式）

3. **指纹测试错误**：测试输入使用错误的语法
   - 修复：将 `__italic__` 改为 `**bold**`
...
### Mermaid 图表设计偏好（2025-07-10）

**用户偏好**：

- 使用 Mermaid 语法绘制架构图，不使用字符画
- 不要在 Mermaid 图表中使用背景颜色（如 `style A fill:#e3f2fd`）
- 保持图表简洁，避免颜色与文字混淆
- 图表语法要正确，避免解析错误
...
```mermaid
SR -.->|write()| OP    SR -.->|监听 dom:operations| DD
```
...
- Mermaid 对中文标签的支持有限，复杂的标签可能导致解析错误
- 去掉样式定义（classDef 和 style）可以让图表更清晰
- 浅色背景配浅色文字会导致可读性问题
...
- 需要在 Mermaid 图表连线上增加描述性文字
- 要求详细说明架构概览的工作流程
- 强调乐观更新时用户是会感知到格式变化的，但避免了符号闪烁

**改进内容**：

- 在架构图的连线上添加详细描述（如 `write()方法调用`、`parser:token事件`）
- 增加架构说明章节，详细解释三层架构的职责和交互流程
- 修正描述：乐观更新时用户会看到格式变化，但关键是避免格式符号的闪烁
- 强调快照渲染器通过构造时依赖注入获得样式处理器
...
<!-- 最后回顾: 2025-07-10T04:40:00+08:00 -->
<!-- V2架构实现完成，采用快照驱动的事件解耦设计 -->
<!-- 更新架构文档，使用 Mermaid 语法替换字符画 -->
<!-- 完善架构文档，增加详细的文字说明和连线描述 -->
