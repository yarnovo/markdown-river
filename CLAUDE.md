...
**关键洞察**：策略的核心作用是决定下一次渲染的内容和解析位置，驱动缓存管理器的行为
...
1. **架构文档更新**：
   - 移除所有"歧义检测"概念
   - 更新为"策略决策"、"渲染决策"
   - 强调策略的核心作用：决定下一次渲染内容和解析位置

2. **代码注释修正**：
   - 更新 MarkdownRiver.ts 中的 buffer 事件原因：从 "Ambiguity detected" 改为 "Strategy decided to wait"

3. **测试文件更新**：
   - 测试描述从"歧义检测测试"改为"策略决策测试"
   - 更新测试用例名称，移除"歧义"术语

4. **研究文档修正**：
   - 更新 marked-flicker-analysis.md 中的策略说明
   - 修正为策略驱动的决策逻辑

**核心理念转变**：

- 旧概念：歧义检测 → 等待解决 → 解析
- 新概念：策略分析 → 智能决策 → 返回位置或补全内容

### 端到端测试框架（2025-07-10晚）

**用户需求**：创建E2E测试，验证实际的用户流程

**测试架构设计**：

1. **目录结构**：
   - `tests/e2e/` - E2E测试主目录
   - `tests/e2e/io/` - JSON格式的测试数据文件
   - 每个JSON文件定义一个完整的用户场景
...
3. **测试实现特点**：
   - 数据驱动：测试逻辑和测试数据分离
   - 真实模拟：模拟用户逐字符输入的真实场景
   - 详细验证：不仅验证最终结果，还验证每个步骤的中间状态
   - 可扩展：轻松添加新的测试场景

**用户测试习惯观察**：

- **一个一个来**：明确要求逐个创建测试用例，不要批量创建
- **中断过度实现**："我不是让你只增加那一个...一个一个来"
- **关注即时反馈**：特别关注乐观更新的即时体验

### E2E测试用例扩展（2025-07-10晚）

**新增测试场景**：

1. **行内代码乐观更新**：
   - 输入：`` ` `` → `` `c ``
   - 行为：第一个字符等待，第二个字符自动补全为 `` `c` ``
...
3. **加粗格式无闪烁测试**（新增）：
   - 输入序列：`*` → `*` → `strong` → `*` → `*`
   - 预期行为：不应显示多余的星号，始终保持加粗显示
   - 测试文件：`strong-format-no-flash.json`

**marked HTML 输出特性**：

- 单段落内换行：`<br>` 标签与后续内容在同一行
- 示例：`<p>Hello world<br><em>h</em></p>\n`（非 `<br>\n<em>`）

### 转换器架构重构（2025-07-10 - 从策略模式到转换器模式）

**用户需求**："我们需要的是一个正则...把markdown过程当中的每一个快照流逝输入的每个快照都经由这个正则表达式去处理"

**架构转变**：

从策略模式（Strategy Pattern）转向转换器模式（Transformer Pattern）：

- **旧架构**：CacheManager → ParseStrategy → 解析位置/内容
- **新架构**：CacheManager → MarkdownTransformer → 转换后内容

**MarkdownTransformer 核心规则**：
...
- `**strong*` 情况：暂时移除最后的 `*`，显示为加粗，避免 marked 解析错误

### 事件驱动架构简化（2025-07-10）

**用户要求**："CacheManager 架构再调整一下再简单一点"

**简化内容**：

- CacheManager 不再维护解析位置和流状态
- 只负责追加内容和发送事件
- MarkdownRiver 监听 `cache:updated` 事件并处理

**事件流程**：

```
write() → CacheManager.append() → emit('cache:updated')
                                        ↓
                          MarkdownRiver.handleCacheUpdate()
                                        ↓
                              transform → parse → emit('content:parsed')
```

### Strong Format Bug 修复（2025-07-10晚）

**问题描述**：输入加粗格式 `**strong**` 过程中会出现多余的星号

**根本原因**：

1. 转换器没有正确处理内容中间出现的格式符号
2. 斜体和加粗的检测逻辑只处理了以符号开头的情况
...
<!-- 最后更新: 2025-07-11T01:05:00+08:00 -->
<!-- 架构重构: 从策略模式转向转换器模式 -->
<!-- 事件驱动: CacheManager简化为纯事件发射器 -->
<!-- Bug修复: Strong format无闪烁，中间位置格式补全 -->
<!-- 测试完善: 105个测试全部通过，包含E2E测试 -->
<!-- 代码质量: ESLint零错误，TypeScript零错误 -->
<!-- 用户交流: 强调直接沟通、简洁实现、用户体验优先 -->
