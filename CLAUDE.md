...
### StandardStrategy 核心算法

1. **列表特殊处理**：单独 `-` 立即渲染，`- ` 智能等待
2. **未闭合格式检测**：扫描行内代码、强调、斜体、链接
3. **智能策略选择**：
   - 无未匹配符号：解析到末尾
   - 符号前有完整内容：优先解析完整部分
...
<!-- 最后回顾: 2025-07-10T22:02:00+08:00 -->
<!-- 压缩完成: 从约2000行压缩到500行，保留所有关键信息 -->
<!-- 压缩比例: 约1/4，结构化压缩，信息密度提升 -->
<!-- 涵盖内容: 架构演进、技术决策、用户偏好、实现细节、经验教训 -->
